...
Many M5Stack devices expose only a generic USB-serial bridge to Windows. That means port
discovery can be healthy while board identification stays `Unknown`.
...
If the COM port appears but the board name is missing, treat that as a board identity gap,
not an automatic driver failure.
...
If `esptool` can identify the chip, you have strong proof that the port is real even if the
board model remains `Unknown`.
